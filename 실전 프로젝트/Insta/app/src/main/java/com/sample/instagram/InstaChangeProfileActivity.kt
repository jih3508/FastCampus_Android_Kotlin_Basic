...
class InstaChangeProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.change_img).setOnClickListener{
            ...
            retrofitService.changePrifile(userId, header, body, user).enqueue(object : Callback<Any>{
    ...
    private fun getReaFile(uri: Uri): File?{
        var uri: Uri? = uri
        ...
        val colum_index: Int = cursor.getColumnIndexOrThrow(MediaStore.Images.Media.DATA)
        cursor.moveToFirst()
        val path: String = cursor.getString(colum_index)
        if(cursor != null){
            cursor.close()
            cursor = null
        }
