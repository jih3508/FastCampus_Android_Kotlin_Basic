...
class Intent_One : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // - URL (Uniform Resource Indentifier)
        //  - id라고 생각을 하면 된다. → 고유하다
        //  -  자원을 나타내는 주소
        //  - URL
        //      - 인턴넷 페이지의 고유한 주소
        ...
        val implicit_one: TextView = findViewById(R.id.intent_one)
        implicit_one.setOnClickListener{
            val intent : Intent = Intent()
        ...
        val startActivityLauncher : ActivityResultLauncher<Intent> = registerForActivityResult(
            ...
            // ActivityResult API
            // - 각각의 intent가 처리되는 곳이 별도로 있다 → 구분이 필요하다
        }
        ...
        (findViewById<TextView>(R.id.intent_six)).apply {
            this.setOnClickListener{
                val intent = Intent(this@Intent_One, Intent_Two::class.java).apply {
                    val imageUri = Uri.parse("android.resource://" + packageName + "/drawable/" + "download")
