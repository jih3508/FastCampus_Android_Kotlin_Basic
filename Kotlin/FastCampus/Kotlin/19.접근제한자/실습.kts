...
fun changeNumber(){
    ...
    number = 20 // 하위 스코프에서 상위 맴버에 접근할 수 있다.
    var number : String = "재정의" // 하위 스코프에서 상위 맴버를 재정의 할 수 있다.
    println(number)
}
changeNumber()
println(number)

//internalNumber = 30 상위 스코프에서 하위 스코프 맴버에 접근할 수 없다.
// 하위에서 상위로 접근은 가능
// 상위에서 하위로 접근 불가능 -> 사실상 하위는 독립적

// 접근제한자
class Number(private var number: Int = 10){
    fun changeNumber(){
        this.number = 100 // Number라는 스코프 안에 존재하기 때문에 number에 접근/변경 할 수 있댜ㅏ.
    }
